...
fn test_multiplier_compilation() {
    ...
    assert!(target.num_constraints() > 0, "Should have at least 1 constraint");
    assert_eq!(target.num_private_inputs(), 2, "Multiplier has 2 inputs (a, b)");
...
fn test_multiplier_proof_generation() {
    ...
    // Generate proof
    let proof = target.prove(&[a.clone(), b.clone()]).expect("Proof generation failed");
    assert!(!proof.is_empty(), "Proof should not be empty");
    
    // Verify proof
    let public_inputs = vec![FieldElement::from_u64(35)]; // 5 * 7 = 35
    let valid = target.verify(&proof, &public_inputs).expect("Verification failed");
...
fn test_arithmetic_boundaries() {
    ...
    let result = target.execute(&[zero.clone(), one.clone()])
        .expect("0 * 1 should work");
    assert_eq!(result[0], zero, "0 * 1 = 0");
    
    // Test with 1
    let result = target.execute(&[one.clone(), one.clone()])
    ...
    let result = target.execute(&[one.clone(), max.clone()])
...
fn test_full_backend_verification() {
    ...
    target.compile().expect("Compilation failed");
    println!("✓ Circuit compiled: {} constraints", target.num_constraints());
