...
fn test_framework_aware_mutator_circom() {
    ...
    prior_inputs.insert(
        "circuit_a".to_string(),
        vec![FieldElement::from_u64(100u64)],
    );
    ...
    for _ in 0..10 {
        ...
        for values in mutated.values() {
            for val in values {
                assert!(
                    is_valid_bn254_field_element(val),
...
fn test_default_vs_circom_mutation_differences() {
    ...
    let (default_result, _) = default_mutator.mutate_inputs(&spec, &prior_inputs, &mut default_rng);
...
fn test_with_framework_builder() {
    let mutator = ChainMutator::new().with_framework(Framework::Circom);
...
fn test_single_step_chain() {
    ...
    let spec = ChainSpec::new("single_step", vec![StepSpec::fresh("only_circuit")]);

    let mut prior_inputs = HashMap::new();
    prior_inputs.insert(
        "only_circuit".to_string(),
        vec![FieldElement::from_u64(42u64)],
    );
...
fn test_boundary_injection_with_framework() {
    let mutator =
        ChainMutator::new_with_framework(Framework::Circom).with_weights(MutationWeights {
...
fn test_mutation_validity_rate() {
    ...
    prior_inputs.insert(
        "circuit_b".to_string(),
        vec![FieldElement::from_u64(200u64)],
    );
    ...
    assert!(
        validity_rate >= 80.0,
        "Validity rate should be at least 80%, got {:.1}%",
        validity_rate
    );
}
