...
use zk_fuzzer::formal::{
    CircuitProperty, CoqExporter, ProofExporter, ProofObligation, ProofSystem, PropertyType,
};
...
#[test]
fn test_export_obligation_renders_addition_in_coq_form() {
    let exporter = CoqExporter::new(
        "21888242871839275222246405745257275088548364400416034343698204186575808495617",
    );
    let obligation = ProofObligation {
    ...
    let result = exporter.export_obligation(&obligation);
    assert!(result.code.contains("(fadd x y)"));
}
