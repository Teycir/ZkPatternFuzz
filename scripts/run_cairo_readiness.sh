...
usage() {
  cat <<'EOF'
Run Cairo full-capacity readiness checks (integration + matrix run).

Usage:
  scripts/run_cairo_readiness.sh [options]

Options:
  --matrix <path>          Matrix YAML (default: targets/zk0d_matrix_cairo_readiness.yaml)
...
MATRIX="targets/zk0d_matrix_cairo_readiness.yaml"
...
export ZKFUZZ_CAIRO_STRICT_COVERAGE="${ZKFUZZ_CAIRO_STRICT_COVERAGE:-1}"
