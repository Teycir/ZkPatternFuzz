...
- [ ] Backend maturity scorecard published and gate enforced:
  ...
  - `./scripts/release_candidate_gate.sh --bench-root artifacts/benchmark_runs --required-passes 2 --required-backends noir,cairo,halo2 --required-maturity-backends circom,noir,cairo,halo2 --min-backend-completion-rate 0.90 --min-backend-selector-matching-total-per-backend noir=25,cairo=4,halo2=4 --min-backend-enabled-targets 5 --min-backend-maturity-score 4.5 --max-backend-runtime-error 0 --max-backend-preflight-failed 0 --max-backend-run-outcome-missing-rate 0.05`
...
- [ ] Release validation workflow run recorded:
  - GitHub Actions `Release Validation` (`workflow_dispatch`) with:
    ...
    - `required_passes=2`
    - readiness + maturity gate inputs set for backend thresholds (`required_backends`, `required_maturity_backends`, `min_backend_completion_rate`, `min_backend_selector_matching_total_per_backend`, `min_backend_enabled_targets`, `min_backend_maturity_score`, `max_backend_runtime_error`, `max_backend_preflight_failed`, `max_backend_run_outcome_missing_rate`)
