...
impl ZkEvmAttack {
    ...
    pub fn run(
        ...
        tracing::info!(
            "zkEVM attack detection complete: {} findings",
            findings.len()
        );
    ...
    fn generate_state_transition_case(
        &mut self,
        base_inputs: &[FieldElement],
    ) -> StateTransitionTest {
    ...
    fn run_opcode_boundary_tests(
        ...
        for opcode in opcodes_to_test {
            ...
            for _ in 0..self.config.opcode_boundary_tests {
                ...
                if let Some(finding) =
                    self.check_opcode_boundary(executor, opcode, &boundary_case)?
                {
                    self.findings.push(finding);
                }
            }
...
impl OpcodeBoundaryCase {
    fn to_inputs(&self) -> Vec<FieldElement> {
        match self {
            ...
            Self::MixedBoundary(count) => (0..*count)
                .map(|i| {
                    if i % 2 == 0 {
            ...
            Self::NearOverflow(count) => (0..*count)
                .map(|_| {
                ...
                if let Some(ref err) = result.error {
                    if !err.contains("depth") && !err.contains("stack") {
                        return Some(ZkEvmTestResult {
                            vulnerability_type: ZkEvmVulnerabilityType::CallHandlingVulnerability,
                            description: format!(
                                "Unexpected error at call depth {}: {}",
                                depth, err
                            ),
                            opcode: Some("CALL".to_string()),
                            witness: nested_calls.clone(),
